...
The aim of this project is to exmaine whether there is an association between the caloric content of recipes and their average user ratings using the data from the TidyTuesday package. Specifically, the hypothesis that recipes with higher calories receive higher average user ratings.
...
all_recipes_c <- all_recipes_c %>%
  filter(between(calories, mean - 3*sd, mean + 3*sd))  #filtering out observatoins ±3 sd
```
